...
public class StringFileDocx extends UiAutomatorTestCase {
	...
	private final static String FILE_NAME = "HR-DEPENDENT REGISTRATION FORM.doc";
	...
	public void testStringFile() throws UiObjectNotFoundException,
			RemoteException {
		loggerd(StringFileDocx.class.getName(), "Start Testing");

		loggerd(StringFileDocx.class.getName(), "Launch FileString App");
		launchFileStringApp(APP_NAME);

		loggerd(StringFileDocx.class.getName(), "Sign In");
		signIn(EMAIL_SIGNIN, PASSWORD);

		loggerd(StringFileDocx.class.getName(), "String a file");
		menuStringFile(STRING_A_FILE);

		loggerd(StringFileDocx.class.getName(), "Select file resource");
		...
		loggerd(StringFileDocx.class.getName(), "Check File Uploaded");
		notificationCheckFileUploaded(FILE_UPLOADED_SUCCESSFUL);

		loggerd(StringFileDocx.class.getName(),
				"Verify File In List All File");
		clickListViewItem(FILE_NAME, ACTION_TAKE_SCREENSHOT);

		loggerd(StringFileDocx.class.getName(), "Sign Out");
		signOut();

		loggerd(StringFileDocx.class.getName(), "Test Complete");
	}
