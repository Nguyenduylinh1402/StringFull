 ...
 */
public class StringFileJpg extends UiAutomatorTestCase {
	...
	private final static String FILE_NAME = "C360_2014-11-28-22-04-59-538.jpg";
	...
	public void testStringFile() throws UiObjectNotFoundException,
			RemoteException {
		loggerd(StringFileJpg.class.getName(), "Start Testing");

		loggerd(StringFileJpg.class.getName(), "Launch FileString App");
		launchFileStringApp(APP_NAME);

		loggerd(StringFileJpg.class.getName(), "Sign In");
		signIn(EMAIL_SIGNIN, PASSWORD);

		loggerd(StringFileJpg.class.getName(), "String a file");
		menuStringFile(STRING_A_FILE);

		loggerd(StringFileJpg.class.getName(), "Select file resource");
		...
		loggerd(StringFileJpg.class.getName(), "Check File Uploaded");
		notificationCheckFileUploaded(FILE_UPLOADED_SUCCESSFUL);

		loggerd(StringFileJpg.class.getName(),
				"Verify File In List All File");
		clickListViewItem(FILE_NAME, ACTION_TAKE_SCREENSHOT);

		loggerd(StringFileJpg.class.getName(), "Sign Out");
		signOut();

		loggerd(StringFileJpg.class.getName(), "Test Complete");
	}
