 ...
 */
public class StringFilePng extends UiAutomatorTestCase {
	...
	private final static String FILE_NAME = "Screenshot tao2.png";
	...
	public void testStringFile() throws UiObjectNotFoundException,
			RemoteException {
		loggerd(StringFilePng.class.getName(), "Start Testing");

		loggerd(StringFilePng.class.getName(), "Launch FileString App");
		launchFileStringApp(APP_NAME);

		loggerd(StringFilePng.class.getName(), "Sign In");
		signIn(EMAIL_SIGNIN, PASSWORD);

		loggerd(StringFilePng.class.getName(), "String a file");
		menuStringFile(STRING_A_FILE);

		loggerd(StringFilePng.class.getName(), "Select file resource");
		...
		loggerd(StringFilePng.class.getName(), "Check File Uploaded");
		notificationCheckFileUploaded(FILE_UPLOADED_SUCCESSFUL);

		loggerd(StringFilePng.class.getName(),
				"Verify File In List All File");
		clickListViewItem(FILE_NAME, ACTION_TAKE_SCREENSHOT);

		loggerd(StringFilePng.class.getName(), "Sign Out");
		signOut();

		loggerd(StringFilePng.class.getName(), "Test Complete");
	}
