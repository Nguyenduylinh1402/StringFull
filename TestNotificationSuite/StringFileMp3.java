 ...
 */
public class StringFileMp3 extends UiAutomatorTestCase {
	...
	private final static String EMAIL_SIGNIN = "sta005@yopmail.com";
	...
	private final static String FILE_NAME = "Tonight I Celebrate My Love For You - Peabo Bryson Roberta Flack.mp3";
	...
	public void testStringFile() throws UiObjectNotFoundException,
			RemoteException {
		loggerd(StringFileMp3.class.getName(), "Start Testing");

		loggerd(StringFileMp3.class.getName(), "Launch FileString App");
		launchFileStringApp(APP_NAME);

		loggerd(StringFileMp3.class.getName(), "Sign In");
		signIn(EMAIL_SIGNIN, PASSWORD);

		loggerd(StringFileMp3.class.getName(), "String a file");
		menuStringFile(STRING_A_FILE);

		loggerd(StringFileMp3.class.getName(), "Select file resource");
		...
		loggerd(StringFileMp3.class.getName(), "Check File Uploaded");
		notificationCheckFileUploaded(FILE_UPLOADED_SUCCESSFUL);

		loggerd(StringFileMp3.class.getName(),
				"Verify File In List All File");
		clickListViewItem(FILE_NAME, ACTION_TAKE_SCREENSHOT);

		loggerd(StringFileMp3.class.getName(), "Sign Out");
		signOut();

		loggerd(StringFileMp3.class.getName(), "Test Complete");
	}
