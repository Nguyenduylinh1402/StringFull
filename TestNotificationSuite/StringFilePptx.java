 ...
 */
public class StringFilePptx extends UiAutomatorTestCase {
	...
	private final static String FILE_NAME = "PL_15_businessethics.ppt";
	...
	public void testStringFile() throws UiObjectNotFoundException,
			RemoteException {
		loggerd(StringFilePptx.class.getName(), "Start Testing");

		loggerd(StringFilePptx.class.getName(), "Launch FileString App");
		launchFileStringApp(APP_NAME);

		loggerd(StringFilePptx.class.getName(), "Sign In");
		signIn(EMAIL_SIGNIN, PASSWORD);

		loggerd(StringFilePptx.class.getName(), "String a file");
		menuStringFile(STRING_A_FILE);

		loggerd(StringFilePptx.class.getName(), "Select file resource");
		...
		loggerd(StringFilePptx.class.getName(), "Check File Uploaded");
		notificationCheckFileUploaded(FILE_UPLOADED_SUCCESSFUL);

		loggerd(StringFilePptx.class.getName(),
				"Verify File In List All File");
		clickListViewItem(FILE_NAME, ACTION_TAKE_SCREENSHOT);

		loggerd(StringFilePptx.class.getName(), "Sign Out");
		signOut();

		loggerd(StringFilePptx.class.getName(), "Test Complete");
	}
