 ...
 */
public class StringFileXlsx extends UiAutomatorTestCase {
	...
	private final static String FILE_NAME = "timetable-PNV2M & 2T_week 36.xls";
	...
	public void testStringFile() throws UiObjectNotFoundException,
			RemoteException {
		loggerd(StringFileXlsx.class.getName(), "Start Testing");

		loggerd(StringFileXlsx.class.getName(), "Launch FileString App");
		launchFileStringApp(APP_NAME);

		loggerd(StringFileXlsx.class.getName(), "Sign In");
		signIn(EMAIL_SIGNIN, PASSWORD);

		loggerd(StringFileXlsx.class.getName(), "String a file");
		menuStringFile(STRING_A_FILE);

		loggerd(StringFileXlsx.class.getName(), "Select file resource");
		...
		loggerd(StringFileXlsx.class.getName(), "Check File Uploaded");
		notificationCheckFileUploaded(FILE_UPLOADED_SUCCESSFUL);

		loggerd(StringFileXlsx.class.getName(),
				"Verify File In List All File");
		clickListViewItem(FILE_NAME, ACTION_TAKE_SCREENSHOT);

		loggerd(StringFileXlsx.class.getName(), "Sign Out");
		signOut();

		loggerd(StringFileXlsx.class.getName(), "Test Complete");
	}
